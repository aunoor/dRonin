...
void PIOS_Board_Init(void)
{
	...
	if (boot_count < 3) {
		...
		HwSprf3SetDefaults(HwSprf3Handle(), 0);
...
#undef PIOS_INCLUDE_USB
	...
	HwSprf3USB_VCPPortGet(&hw_usb_vcpport);

	if (!usb_cdc_present) {
		/* Force VCP port function to disabled if we haven't advertised VCP in our USB descriptor */
		hw_usb_vcpport = HWSPRF3_USB_VCPPORT_DISABLED;
	}
	...
	HwSprf3USB_HIDPortGet(&hw_usb_hidport);

	if (!usb_hid_present) {
		/* Force HID port function to disabled if we haven't advertised HID in our USB descriptor */
		hw_usb_hidport = HWSPRF3_USB_HIDPORT_DISABLED;
	}
	...
	HwSprf3DSMxModeOptions hw_DSMxMode;
	HwSprf3DSMxModeGet(&hw_DSMxMode);

	uint8_t hw_uart3;
	HwSprf3Uart3Get(&hw_uart3);
...
#include "pios_bmp280_priv.h"
	uint8_t hw_baro;
	HwSprf3BarometerGet(&hw_baro);
	switch(hw_baro) {
		case HWSPRF3_BAROMETER_BMP280:
	...
	HwSprf3MagnetometerGet(&hw_mag);
	HwSprf3ExtMagOrientationGet(&hw_orientation);

	// XXX TODO MIXING ENUMS HERE NOT SAFE/CORRECT
	switch(hw_mag) {
		case HWSPRF3_MAGNETOMETER_INTERNAL:
			pios_mpu_cfg.use_internal_mag=true;
			break;
		case HWSPRF3_MAGNETOMETER_NONE:
			pios_mpu_cfg.use_internal_mag=false;
			break;
		case HWSPRF3_MAGNETOMETER_EXTERNALHMC5883:
			pios_mpu_cfg.use_internal_mag=false;
#if defined(PIOS_INCLUDE_I2C)
			PIOS_WDG_Clear();
...
#endif // PIOS_INCLUDE_I2C
			break;
		case HWSPRF3_MAGNETOMETER_EXTERNALHMC5983:
			pios_mpu_cfg.use_internal_mag=false;
	...
	HwSprf3GyroRangeOptions hw_gyro_range;
	HwSprf3GyroRangeGet(&hw_gyro_range);

	switch(hw_gyro_range) {
		case HWSPRF3_GYRORANGE_250:
			PIOS_MPU_SetGyroRange(PIOS_MPU_SCALE_250_DEG);
			break;
		case HWSPRF3_GYRORANGE_500:
			PIOS_MPU_SetGyroRange(PIOS_MPU_SCALE_500_DEG);
			break;
		case HWSPRF3_GYRORANGE_1000:
			PIOS_MPU_SetGyroRange(PIOS_MPU_SCALE_1000_DEG);
			break;
		case HWSPRF3_GYRORANGE_2000:
			PIOS_MPU_SetGyroRange(PIOS_MPU_SCALE_2000_DEG);
			break;
	}

	HwSprf3AccelRangeOptions hw_accel_range;
	HwSprf3AccelRangeGet(&hw_accel_range);
	switch(hw_accel_range) {
		case HWSPRF3_ACCELRANGE_2G:
			PIOS_MPU_SetAccelRange(PIOS_MPU_SCALE_2G);
			break;
		case HWSPRF3_ACCELRANGE_4G:
			PIOS_MPU_SetAccelRange(PIOS_MPU_SCALE_4G);
			break;
		case HWSPRF3_ACCELRANGE_8G:
			PIOS_MPU_SetAccelRange(PIOS_MPU_SCALE_8G);
			break;
		case HWSPRF3_ACCELRANGE_16G:
			PIOS_MPU_SetAccelRange(PIOS_MPU_SCALE_16G);
			break;
	}

	HwSprf3MPU9250GyroLPFOptions hw_mpu_dlpf;
	HwSprf3MPU9250GyroLPFGet(&hw_mpu_dlpf);
	uint16_t bandwidth = \
		(hw_mpu_dlpf == HWSPRF3_MPU9250GYROLPF_184) ? 184 : \
		(hw_mpu_dlpf == HWSPRF3_MPU9250GYROLPF_92)  ? 92  : \
		(hw_mpu_dlpf == HWSPRF3_MPU9250GYROLPF_41)  ? 41  : \
		(hw_mpu_dlpf == HWSPRF3_MPU9250GYROLPF_20)  ? 20  : \
		(hw_mpu_dlpf == HWSPRF3_MPU9250GYROLPF_10)  ? 10  : \
		(hw_mpu_dlpf == HWSPRF3_MPU9250GYROLPF_5)   ? 5   : \
		188;
